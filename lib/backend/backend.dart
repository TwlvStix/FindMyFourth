...
/// Functions to query AddPlayersRecords (as a Stream and as a Future).
Future<int> queryAddPlayersRecordCount({
...
Stream<List<AddPlayersRecord>> queryAddPlayersRecord({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
...
Future<List<AddPlayersRecord>> queryAddPlayersRecordOnce({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
...
Future<FFFirestorePage<AddPlayersRecord>> queryAddPlayersRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<AddPlayersRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
